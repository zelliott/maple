...
def make_dfs():
	...
	for domain_file_name in os.listdir('corpora'):
		...
		np.save('./df_models/' + domain_file_name + '.npy', domain_dfs)

	print("making entire corpus dfs map")
	np.save('./df_models/whole_corpus.npy', all_dfs)
...
def get_most_difficult(doc, doc_class):
	doc = doc.lower()
	dfs_class = np.load('./df_models/' + doc_class + '.npy').item()
	num_doc_class = num_docs[doc_class]
	dfs_whole = np.load('./df_models/whole_corpus.npy').item()
